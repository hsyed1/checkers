...
public class LocalBot {
    ...
    {
      //hashmap sets key=jumpToPosition and value=JumpFromPosition  
        ...
        if(pos%8==1 || pos%8==2 || pos%8==3 || pos%8==4)
        {
          System.out.println("Pos: "+pos);
            if(pos+4<=32 && (b.boardVal[pos+4]==otherKing || b.boardVal[pos+4]==otherPawn))
            {
                ...
                //if(pos==12)
                  System.out.println("Pos: "+pos+" midPos: "+pos+4+" jump: "+jump);
      ...
      System.out.println("return val: "+jumpPos);
        return jumpPos;
    }
            ...
            if(pos+4<=32)
            {
                if(b.boardVal[pos+4]==WHITE_PAWN || b.boardVal[pos+4]==WHITE_KING)
                {
                  int jump=jumpFromTo(pos, pos+4);
                    if((jump>0 && jump<=32) && b.boardVal[jump]==NO_PIECE)
                    {
                      //jumpSeq=jumpSeq+","+Integer.toString(pos)+","+Integer.toString(jump);
                        jumpPos.put(jump,pos+4);
                        MyBoard b1=new MyBoard(b.boardVal);
                        b1.boardVal[pos]=NO_PIECE;
                        b1.boardVal[pos+4]=NO_PIECE;
    ...
    public static LinkedList<MyBoard> getBoards(MyBoard b, String player)
    {
        ...
        for(int pos=1;pos<=32;pos++)
        {
            if("first".equals(player) && b.boardVal[pos]!=NO_PIECE  && b.boardVal[pos]==BLACK_PAWN)
            {
                ...
                {
                  if(tempJump.containsKey(pos+7))
                  {
                    int jump=pos+7;
                    int midPos=tempJump.get(pos+7);
                    seq.add(jump);
                    newBoard.boardVal[pos]=NO_PIECE;
                    newBoard.boardVal[midPos]=NO_PIECE;
                    if(pos+7==29 || pos+7==30 || pos+7==31 || pos+7==32)
                        newBoard.boardVal[pos+7]=BLACK_KING;
                    else
                        newBoard.boardVal[pos+7]=BLACK_PAWN;
                    
                    tempJump.remove(pos+7);
                    pos=pos+7;
                  }
                  else if(tempJump.containsKey(pos+9))
                  {
                    //seq.add(pos);
                    int jump=pos+9;
                    int midPos=tempJump.get(pos+9);
                    seq.add(jump);
                    newBoard.boardVal[pos]=NO_PIECE;
                    newBoard.boardVal[midPos]=NO_PIECE;
                    if(pos+9==29 || pos+9==30 || pos+9==31 || pos+9==32)
                        newBoard.boardVal[pos+9]=BLACK_KING;
                    else
                        newBoard.boardVal[pos+9]=BLACK_PAWN;
                    
                    tempJump.remove(pos+9);
                    pos=pos+9;
                  }
                  else
                  {
                    set=true;
                    newBoard.setSeq(seq);
                    //System.out.println("Seq of Moves: "+Arrays.toString(seq.toArray())); 
            ...
            if("first".equals(player) && b.boardVal[pos]!=NO_PIECE && b.boardVal[pos]==BLACK_KING)
            {
                ...
                while(!tempJump.isEmpty())
                {
                  ...
                  else
                  {
                    ...
                    newBoard.setSeq(seq);
                    //System.out.println("Seq of Moves: "+Arrays.toString(seq.toArray())); 
                    //System.out.println("Moves from object: "+newBoard.getSeq()); 
                    boardsReached.add(newBoard);
                    break;
                  }
                }
  ...
  for(int pos=1;pos<=32;pos++)
        {  
            ...
            {
                jumpPos=getPawnJump(b, pos, player);
                //System.out.println("JumpPos: "+jumpPos);
                HashMap<Integer,Integer> tempJump=new HashMap<>(jumpPos);
                newBoard=new MyBoard(b.boardVal);
                ArrayList<Integer> seq=new ArrayList<>();
                boolean set=false;
  ...
  for(int pos=1;pos<=32;pos++)
        {
            if("second".equals(player) && b.boardVal[pos]!=NO_PIECE && b.boardVal[pos]==WHITE_KING)
            {
                ...
                System.out.println("King jumps: "+jumpPosKing);
                HashMap<Integer,Integer> tempJump=new HashMap<>(jumpPosKing);
                newBoard=new MyBoard(b.boardVal);
                ArrayList<Integer> seq=new ArrayList<>();
                boolean set=false;
  ...
  for(int pos=1;pos<=32;pos++)
    ...
    public static void main(String[] args) 
    {
        ...
        /*int[] board={5, BLACK_PAWN, BLACK_PAWN, NO_PIECE, BLACK_PAWN, 
                    BLACK_PAWN, BLACK_PAWN, WHITE_KING, WHITE_PAWN, 
                    NO_PIECE, BLACK_KING, NO_PIECE, BLACK_PAWN,
                    NO_PIECE, NO_PIECE, WHITE_KING, WHITE_PAWN, 
                    NO_PIECE, BLACK_PAWN, NO_PIECE, NO_PIECE, 
                    WHITE_PAWN, NO_PIECE, NO_PIECE, WHITE_PAWN, 
                    NO_PIECE, BLACK_PAWN, BLACK_KING, WHITE_PAWN, 
                    WHITE_PAWN, WHITE_PAWN, WHITE_PAWN, NO_PIECE};*/
      
      int[] board={5, WHITE_KING, NO_PIECE, NO_PIECE, BLACK_PAWN, 
                    NO_PIECE, NO_PIECE, BLACK_PAWN, BLACK_PAWN, 
                    NO_PIECE, NO_PIECE, BLACK_PAWN, BLACK_PAWN, 
                    BLACK_PAWN, BLACK_PAWN, NO_PIECE, NO_PIECE, 
                    NO_PIECE, NO_PIECE, NO_PIECE, WHITE_PAWN, 
                    NO_PIECE, NO_PIECE, NO_PIECE, NO_PIECE, 
                    NO_PIECE, NO_PIECE, WHITE_PAWN, WHITE_PAWN, 
                    BLACK_KING, NO_PIECE, WHITE_PAWN, WHITE_PAWN};
        ...
        for(int i = 0; i < boardsReached.size(); i++)    
        {
            ...
            System.out.println(Arrays.toString((boardsReached.get(i)).getSeq().toArray()));
            System.out.println();
        }
        int maxIndex = 0;
        //boolean MAX=true;
